...
import finetune_OPenAI_API as finetuneOpenAIAPI
import finetune_openAi_text_davinci002 as finetuneopenaitdvc002
import fineTuneOpenAIModel as finetuneopenaimodel
...
import openAI_API_using_APIkey2 as apikeyv2
import openAI_API_using_Breaber_Token as openai_token
import openAI_API_usingAPI_text_davinci_003 as openai_text
import phobert as PhoBERT
import phobert2
import phobert3
import phobert4
import phobert_32_50
import phobert_32_100
import phobert_32_150
import phobert_64_50
import phobert_64_100
import rapidapi1
import rapidapi2
import rapidapi3
...
def render_sidebar_rapidapi_config_tab(zone):
   ...
   st.session_state["params"]["model_rapid"] = zone.selectbox(
        "Vui lòng chọn loại mô hình Rapid API bạn muốn sử dụng!!",
...
def render_sidebar_huggingface_config_tab(zone):
   ...
   if st.session_state["params"]["model_4"] == "Model LLama2":
       st.session_state["params"]["model_llama_4"] = zone.selectbox(
        "Vui lòng chọn hình thức sử dụng của mô hình LLama",
...
def render_sidebar():
    ...
    tab_gpt,tab_rapid, tab_prompt, tab_bard, chatgpt_mykey,model_finetune,huggingface = chat_config_expander.tabs(
        ['🌐 ChatBot','❄️ Rapid API','👥 Hộp thoại gợi ý', '🌏 Google Bard', '📚 ChatGPT use APIKey',"🛠️ Model FineTune","🤗 Hugging Face"]
    )
    ...
    if tab_gpt.checkbox('🌐 ChatGPT',help = "Tích 2 lần để sử dụng ChatGPT gôm có Google Bard,OpenAI và LLama2", value=(session_state.selected_tab == 'ChatGPT')):
    ...
    if tab_prompt.checkbox('👥 Prompt',help = "Tích 2 lần để sử dụng Hộp thoại gợi ý", value=(session_state.selected_tab == 'Prompt')):
        ...
        session_state.rapidapi_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if tab_bard.checkbox('🌏 Google Bard',help = "Tích 2 lần để sử dụng Bard với khóa cá nhân", value=(session_state.selected_tab == 'Google Bard')):
        ...
        session_state.huggingface_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if chatgpt_mykey.checkbox('📚 ChatGPT APIKey',help = "Tích 2 lần để sử dụng các tính năng của OpenAI với khóa cá nhân của bạn", value=(session_state.selected_tab == 'ChatGPT APIKey')):
        ...
        session_state.huggingface_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if model_finetune.checkbox('🛠️ Model FineTune',help = "Tích 2 lần để sử dụng Mô hình FineTune gồm có mô hình XLM-R,PhoBert và OpenAI theo mô hình gpt-3.5-turbo", value=(session_state.selected_tab == 'Model FineTune')):
        session_state.selected_tab = 'Model FineTune'
        session_state.gpt_checked = False
        session_state.prompt_checked = False
        session_state.rapidapi_checked = False
        session_state.bard_checked = False
        ...
        session_state.huggingface_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if huggingface.checkbox('🤗 Hugging Face',help = "Tích 2 lần để sử dụng các module lấy từ Hugging Face",value=(session_state.selected_tab == 'HuggingFace')):
        ...
        session_state.finetune_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if tab_rapid.checkbox('❄️ Rapid API',help = "Tích 2 lần để sử dụng Rapid API gồm có BingChat, ChatGPT và Mô hình kết hợp", value=(session_state.selected_tab == 'Rapid API')):
...
def render_last_answer7(question,chat,zone):
    ...
    if st.session_state["params"]["model_rapid"] in {'Harleychatbot_translate'}:
       with st.spinner("Chờ phản hồi..."):
        ...
        tts.play_text_to_speech(answer)
...
def render_last_answer5(question,chat,zone):
    answer_zone = zone.empty()
    chat["messages"].append(question)
    chat["question"].append(question)
    response = ""
    question = ' '.join(question.split())
    if st.session_state["params"]["model_2"] in {'FineTune_OpenAI'}:
        if st.session_state["params"]["model_openai_2"] in {'gpt-3.5-turbo'}:
         with st.spinner("Chờ phản hồi..."):
          temp = st.session_state["params"]["temperature_2"]
          token = st.session_state["params"]["max_tokens_2"]
          pre = st.session_state["params"]["presence_penalty_2"]
          fre = st.session_state["params"]["frequency_penalty_2"]
          top_p = st.session_state["params"]["top_p_2"]
          stream = st.session_state["params"]["stream_2"]
          model_value = st.session_state["params"]["model_openAI_API_1"]
          response = get_finetune_model_openapi(question,model_value,temp,pre,fre,token,top_p,stream)
          answer = ""
          answer = response
          chat["answer"].append(answer)
          chat["messages"].append({"role": "assistant", "content": answer})
          render_ai_message(answer, answer_zone)
          tts.play_text_to_speech(answer)
        elif st.session_state["params"]["model_openai_2"] in {'text-davinci-002'}:
          with st.spinner("Chờ phản hồi..."):
           token = st.session_state["params"]["max_tokens_3"]
           temp = st.session_state["params"]["temperature_3"]
           stp = ""
           stp = st.session_state["params"]["stop_2"]
           oriID = "org-hcK4LtTu8qsTOnJ2Efa16IVj"
           key = "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"
           model = st.session_state["params"]["model_openAI_API_2"]
           response = get_finetune_model_davinci(question,model,key,oriID,stp,token,temp)
           answer = ""
           answer = response
           chat["answer"].append(answer)
           chat["messages"].append({"role": "assistant", "content": answer})
           render_ai_message(answer, answer_zone)
           tts.play_text_to_speech(answer)
    elif st.session_state["params"]["model_2"] in {'FineTune_PhoBERT'}:
     if st.session_state["params"]["model_phobert"] in {'Version'}:
      if st.session_state["params"]["model_phobert_version"] in {'Version1'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = PhoBERT.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_version"] in {'Version2'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert2.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_version"] in {'Version3'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert3.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_version"] in {'Version4'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert4.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
     if st.session_state["params"]["model_phobert"] in {'FineTune 32 batch'}:
      if st.session_state["params"]["model_phobert_32"] in {'Model_32_batch 150_epoch'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert_32_150.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_32"] in {'Model_32_batch 100_epoch'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert_32_100.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_32"] in {'Model_32_batch 50_epoch'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert_32_50.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
     if st.session_state["params"]["model_phobert"] in {"FineTune 64 batch"}:
      if st.session_state["params"]["model_phobert_64"] in {'Model_64_batch 50_epoch'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert_64_50.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
      if st.session_state["params"]["model_phobert_64"] in {'Model_64_batch 100_epoch'}:
       with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        response = phobert_64_100.get_bot_response(processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        tts.play_text_to_speech(answer)
    elif st.session_state["params"]["model_2"] in {'Finetune_XLM_Roberta'}:
      with st.spinner("Chờ phản hồi..."):
        processed_question = ' '.join(question.split())
        context =st.session_state["params"]["text2"]
        response = xlm.get_answer(context,processed_question)
        answer = ""
        answer = response
        chat["answer"].append(answer)
        chat["messages"].append({"role": "assistant", "content": answer})
...
def render_last_answer3(question,chat,zone):
        with st.spinner("Chờ phản hồi..."):
         ...
         answer = ""
         response = config_bard.initialize_bard_session(psid,psidts,psidcc,question)
         print(psid,psidts,psidcc,question)
         answer = response
         chat["answer"].append(answer)
         chat["messages"].append({"role": "assistant", "content": answer})
         render_ai_message(answer, answer_zone)
...
def render_last_answer2(question, chat, zone):
    ...
    if st.session_state["params"]["model"] in {'chatbot-fake'}:
      with st.spinner("Chờ phản hồi..."):
        ...
        chat["messages"].append({"role": "assistant", "content": answer})
        render_ai_message(answer, answer_zone)
        audio_thread = tts.text_to_speech_and_play(answer)
     ...
     with st.spinner("Chờ phản hồi..."):
        ...
        if st.session_state["params"]["model_openai"] in {'Use_API_Key'}:
         with st.spinner("Chờ phản hồi..."):
          ...
          temp = st.session_state["params"]["temperature"]
          token = st.session_state["params"]["max_tokens"]
          pre = st.session_state["params"]["presence_penalty"]
          fre = st.session_state["params"]["frequency_penalty"]
          top_p = st.session_state["params"]["top_p"]
          stream  =  st.session_state["params"]["stream"]
          response = get_openai_response_api_key_v2(keyOpenAI,model_value,question,temp,top_p,stream,pre,token,fre)
          answer = ""
          answer = response
          chat["answer"].append(answer)
          chat["messages"].append({"role": "assistant", "content": answer})
...
def render_chat(chat_name):
    def handle_ask():
        if st.session_state['input']:
            ...
            if st.session_state.selected_tab == 'Model FineTune':
                render_last_answer5(st.session_state["input"],chat,last_answer_zone)
    ...
    def handle_ask1():
            re_generate_zone.empty()
            if st.session_state.selected_tab in ['ChatGPT','Prompt'] :
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer2(input1, chat, last_answer_zone)
            if st.session_state.selected_tab == 'Google Bard' and st.session_state["params"]["1_PSID"] not in["None"]:
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer3(input1,chat,last_answer_zone)
            if st.session_state.selected_tab == 'ChatGPT APIKey' and st.session_state["params"]["api_key1"]not in["None"]:
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer4(input1,chat,last_answer_zone)
            if st.session_state.selected_tab == 'Model FineTune':
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer5(input1,chat,last_answer_zone)
            if st.session_state.selected_tab == 'HuggingFace':
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer6(input1,chat,last_answer_zone)
            if st.session_state.selected_tab == 'Rapid API':
                input = stt.record_and_transcribe()
                input1 = input[0]
                render_user_message(input1, last_question_zone)
                render_stop_generate_button(stop_generate_zone)
                render_last_answer7(input1,chat,last_answer_zone)
    def stop_stt():
        tts.stop_stt()
    ...
    col1.text_area("👻 Bạn: ",
                   key="input",
                   max_chars=4000,height=148,
                   label_visibility='collapsed')
    with col2.container():
        st.form_submit_button("🚀", on_click=handle_ask,help="Sử dụng 🚀 để gửi tin nhắn bằng văn bản")
        st.form_submit_button("🎙️", on_click=handle_ask1,help="Sử dụng 🎙️ để dùng giọng nói với thời gian là 15s")
        st.form_submit_button("🔇", on_click=stop_stt,help="Sử dụng 🔇 để tắt âm thanh phát ra từ phản hồi")
...
def get_openai_response_api_key(messages,model,apikey):
    ...
    return response
def get_openai_response_api_key_v2(apikey,model,messages,temperature,top_p,stream,pre,fre,token):
    response = apikeyv2.generate_chat_completion(apikey,model,messages,temperature,top_p,stream,pre,fre,token)
    return response
...
def get_openai_response_text_davinci(messages,token,keyOpenAIdvc,originalID,temp,stop):
    ...
    return response
def get_finetune_model_openapi(messages,model,temperature,pre,fre,token,top_p,stream):
    response = finetuneopenaimodel.generate_chat_completion(model,messages,temperature,top_p,stream,pre,token,fre)
    return response
def get_finetune_model_davinci(messages,model,key,id,stop,token,tmp):
    response = finetuneopenaitdvc002.main(messages,model,token,key,stop,id,tmp)
    return response
