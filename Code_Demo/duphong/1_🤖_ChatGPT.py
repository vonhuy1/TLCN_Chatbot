...
def render_sidebar():
    ...
    tab_gpt, tab_prompt, tab_bard, chatgpt_mykey,model_finetune,huggingface = chat_config_expander.tabs(
        ['🌐  ChatBot', '👥 Hộp thoại gợi ý', '🌏  Google Bard', '📚  ChatGPT use APIKey',"🛠️ Model FineTune","🤗 Hugging Face"]
    )
    ...
    if chatgpt_mykey.checkbox('📚 ChatGPT APIKey', value=(session_state.selected_tab == 'ChatGPT APIKey')):
        ...
        session_state.huggingface_checked = False
        st.write(f"Đã chọn tab: {session_state.selected_tab}")
    if model_finetune.checkbox('🛠️ Model FineTune', value=(session_state.selected_tab == 'Model FineTune')):
